            ...
            @if ($show_table)
                ...
                <x-mary-table class="table-sm" :headers="$headers" :rows="$categoriesOcurrences" :sort-by="$sortBy" striped
                    ...
                    @scope('cell_actions', $categoryOcurrence)
                        <x-mary-button class="btn-warning text-bold btn-sm" icon="o-pencil" wire:click="edit({{$categoryOcurrence->id}})">Editar
                        </x-mary-button>
                        <x-mary-button class="{{ $categoryOcurrence->active ? 'btn-error' : 'btn-success' }} text-bold btn-sm"
                            icon="o-arrows-right-left" wire:click="toggle({{ $categoryOcurrence->id }})">
                            {{ $categoryOcurrence->active ? 'Desativar' : 'Ativar' }}
                        </x-mary-button>
                    @endscope
                </x-mary-table>
            @endif
